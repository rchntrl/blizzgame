...
class BlizzgameNewsExtension extends DataExtension {
    ...
    public function onBeforeWrite() {
        ...
        /** @var News $news */
        $news = $this->owner;
        $page = SiteTree::get_one("NewsHolderPage", "\"SiteTree\".\"SubsiteID\" = 0"); //1640
        $this->owner->NewsHolderPages()->add($page);
        $page = null;
        foreach ($news->Tags() as $tag) {
            switch ($tag->Title) {
                case 'warcraft':
                    $page = SiteTree::get_one("NewsHolderPage", "\"SiteTree\".\"SubsiteID\" = 1"); //1519
                    break;
                case 'diablo':
                    $page = SiteTree::get_one("NewsHolderPage", "\"SiteTree\".\"SubsiteID\" = 2"); //1639
                    break;
                case 'starcraft':
                    $page = SiteTree::get_one("NewsHolderPage", "\"SiteTree\".\"SubsiteID\" = 3"); //1522
                    break;
            }
            if ($page) {
